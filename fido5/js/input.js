...
export class Input {
  ...
  /* The movement stick.
     ...
     Both are also held, so a slide continues while it is and a jump gets its
     variable height. Jump is on a button too; the stick is the alias that
     lets the left thumb do it while the right holds fire. */
  bindStick(zone, base, knob) {
    ...
    const apply = (e) => {
      ...
      /* Up jumps again, alongside the button. It needs a firmer push than the
         button needs a tap — the threshold is well past the dead zone — so a
         thumb drifting up while turning does not produce one, but a deliberate
         flick does. Held as well as pressed, so the stick gets the same
         variable height the button does: flick for a hop, hold for the full
         jump. */
      if (ny < -FIRE && upArmed) { this.press('jump'); upArmed = false; }
      if (ny > -REARM) upArmed = true;
      this.pointerHeld.jump = ny < -FIRE;
