...
/* ---- Pixel font ---------------------------------------------------------
   4x5 glyphs in a 5x6 cell. Used for damage numbers, loot labels, FiDo-5's
   dialogue, the contextual prompts and both boss title cards.

   It was 3x5, and three pixels is not enough for the letters that are built
   from diagonals. N, M and W all collapsed onto H — THE WARDEN read as THE
   HARDEN — because at three columns the only thing separating them is which
   row the crossbar sits on. A fourth column buys a real diagonal for N, and
   enough asymmetry for M and W to stop collapsing onto H.

   Moving a bar up or down a row is not enough on its own — the eye reads two
   uprights and a bar as an H wherever the bar sits, which is exactly how the
   3x5 set failed, and trying it here failed the same way twice. What the
   fourth column actually buys is a step: a pixel out of one middle column and
   then the other, which is as close to a real diagonal as this size gets. So
   H keeps the single centred bar and is the only one of the four with a bar
   at all, while M, N and W each get a step, told apart by where it sits and
   which way it leans:

       H  bar, row 2      M  step, rows 0-1, leaning left
       N  step, rows 1-2, leaning right      W  step, rows 2-3, leaning left

   M and W lean the same way, so they are put at opposite ends of the glyph;
   N leans the other way, so it can sit between them — and beside W, which in
   SHUTDOWN it does.

   The fourth column also separates 0 from O and U from V.

   Every glyph is exactly 4x5; _textCanvas reads the row length rather than
   assuming, so a future change of width is one constant and this table. */
const GLYPHS = {
  A: ['.##.', '#..#', '####', '#..#', '#..#'],
  B: ['###.', '#..#', '###.', '#..#', '###.'],
  C: ['.###', '#...', '#...', '#...', '.###'],
...
export const GLYPH_W = 5, GLYPH_H = 6;
